...
1.  Install JAGS on your computer, and rerun the installation process as before.
2.  Rerun the analysis, and answer `no` the next time you are asked if `rjags` should be installed. This may affect the `sucra` function of the `dmetar` package because the `gemtc` package cannot be used but all other packages keep their functionality.
...
The `dmetar` packge includes several functions to calculate effect sizes needed for meta-analyses: [`NNT`](https://dmetar.protectlab.org/reference/NNT.html), [`se.from.p`](https://dmetar.protectlab.org/reference/se.from.p.html) and [`pool.groups`](https://dmetar.protectlab.org/reference/pool.groups.html).
