...
The second, [`sucra`](www.dmetar.protectlab.org/reference/sucra.html), calculates the $SUCRA$ for each treatment when provided with a `gemtc` network meta-analysis results object, or a matrix containing rank probabilities. In this example, we will use `dmetar`s in-built `NetDataGemtc` data set.
